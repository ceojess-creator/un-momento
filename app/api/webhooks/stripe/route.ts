    ...
    try {
      ...
      const { data: order, error: orderError } = await supabase
        ...
        .insert({
          ...
          ship_address:     meta.ship_address      || (session as any).shipping?.address?.line1 || '',
          ship_city:        meta.ship_city         || (session as any).shipping?.address?.city  || '',
          ship_state:       meta.ship_state        || (session as any).shipping?.address?.state || '',
          ship_zip:         meta.ship_zip          || (session as any).shipping?.address?.postal_code || '',
