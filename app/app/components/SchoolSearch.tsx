    ...
    <div ref={wrapRef} style={{ position: 'relative', width: '100%' }}>
      <div style={{ position: 'relative' }}>
        ...
        <div style={{
          ...
          {loading    ? '⏳'
           : selected ? <span onClick={clear} style={{ cursor:'pointer', color:'#4ADE80' }}>✓</span>
           : query    ? <span onClick={clear} style={{ cursor:'pointer' }}>✕</span>
           : '🔍'}
        </div>
      </div>
      ...
      {selected && (
        <div style={{
          ...
          <button onClick={clear} style={{
            ...
            color: muted, cursor: 'pointer', fontSize: 12,
            padding: '0 4px',
          }}>Change</button>
        </div>
      )}
      ...
      {open && results.length === 0 && query.length >= 2 && !loading && (
        <div style={{
          position: 'absolute', top: '100%', left: 0, right: 0,
          background: dropBg, border: `1px solid ${border}`,
          borderRadius: 8, padding: '12px', zIndex: 1000,
          marginTop: 4, fontSize: 12, color: muted,
          boxShadow: '0 4px 16px rgba(0,0,0,0.12)',
        }}>
          No schools found for "{query}". Try a different spelling or city name.
        </div>
      )}
    </div>
  );
