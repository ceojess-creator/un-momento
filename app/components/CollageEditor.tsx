...
const TEMPLATES = [
  ...
  {
    id: 'two_side', label: '2 side by side',
    slots: [{ x: 0, y: 0, w: 0.5, h: 1 }, { x: 0.5, y: 0, w: 0.5, h: 1 }],
  },
  {
    id: 'two_stack', label: '2 stacked',
...
interface CollageEditorProps {
  onComplete:       (dataUrl: string, slots: any[]) => void;
  onBack:           () => void;
...
export default function CollageEditor({
  onComplete, onBack,
  defaultGradName = '', defaultSchool = '',
}: CollageEditorProps) {
  const wrapRef      = useRef<HTMLDivElement>(null);
  const canvasRef    = useRef<HTMLCanvasElement>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const dragRef      = useRef<{ ov: Overlay; sx: number; sy: number } | null>(null);
  ...
  const [slots,       setSlots]       = useState<SlotData[]>(
    Array.from({ length: 6 }, () => ({ img: null, filter: 'none', zoom: 1, panX: 0, panY: 0 }))
  );
  ...
  // Get canvas pixel dimensions based on wrapper width
  function getDims() {
    const wrap = wrapRef.current;
    const W    = wrap ? wrap.clientWidth : 360;
    return orientation === 'l'
      ? { W, H: Math.round(W * 2 / 3) }
      : { W: Math.round(W * 2 / 3), H: W };
  }
  ...
  const draw = useCallback(() => {
    ...
    tpl.slots.forEach((slot, i) => {
      const x = Math.round(slot.x * W) + GAP;
      const y = Math.round(slot.y * H) + GAP;
      const w = Math.round(slot.w * W) - GAP * 2;
      const h = Math.round(slot.h * H) - GAP * 2;
      ...
      if (s?.img) {
        ...
        let scale = (imgAspect > slotAspect ? h / ih : w / iw) * s.zoom;
        const dw = iw * scale;
        const dh = ih * scale;
        const dx = x + (w - dw) / 2 + s.panX;
        const dy = y + (h - dh) / 2 + s.panY;
    ...
    // Border strip / QR
    const STRIP = Math.round(H * 0.1);
    if (qr === 'border') {
      ctx.fillStyle = 'rgba(255,255,255,0.97)';
      ctx.fillRect(0, H - STRIP, W, STRIP);
      ctx.fillStyle    = '#333';
      ctx.font         = `${Math.round(STRIP * 0.38)}px Arial`;
      ctx.textAlign    = 'left';
      ctx.textBaseline = 'middle';
      const label = gradName
        ? `${gradName}${school ? ` · ${school}` : ''} · unmomentoprints.com`
        : 'unmomentoprints.com';
      ctx.fillText(label, 8, H - STRIP + STRIP / 2, W - STRIP - 12);
      const QS = STRIP - 4;
      ctx.fillStyle = '#000';
      ctx.fillRect(W - QS - 2, H - STRIP + 2, QS, QS);
      ctx.fillStyle    = '#fff';
      ctx.font         = `${Math.round(QS * 0.3)}px Arial`;
      ctx.textAlign    = 'center';
      ctx.fillText('QR', W - QS/2 - 2, H - STRIP + STRIP/2);
    }

    if (qr === 'corner_br' || qr === 'corner_bl') {
      const QS = Math.round(H * 0.13);
  ...
  function handleCanvasClick(e: React.MouseEvent<HTMLCanvasElement>) {
    ...
    for (let i = 0; i < tpl.slots.length; i++) {
      ...
      if (x >= sx && x <= sx+sw && y >= sy && y <= sy+sh) {
        setActiveSlot(i);
        if (!slots[i].img) fileInputRef.current?.click();
        return;
      }
    }
  }
  ...
  function handleMouseDown(e: React.MouseEvent<HTMLCanvasElement>) {
    ...
    for (let i = overlays.length - 1; i >= 0; i--) {
      const ov = overlays[i];
      if (Math.abs(x - ov.x) < 40 && Math.abs(y - ov.y) < 40) {
        dragRef.current = { ov, sx: x - ov.x, sy: y - ov.y };
        setSelected(ov.id);
        return;
      }
    }
  ...
  function handleMouseMove(e: React.MouseEvent<HTMLCanvasElement>) {
    ...
    const { x, y } = canvasCoords(e);
    const { ov, sx, sy } = dragRef.current;
    setOverlays(prev => prev.map(o =>
      o.id === ov.id ? { ...o, x: x - sx, y: y - sy } : o
    ));
  }
  ...
  async function handlePhotoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    img.onload = () => {
      setSlots(prev => prev.map((s, i) =>
        i === activeSlot ? { ...s, img } : s
      ));
      const next = slots.findIndex((s, i) => i > activeSlot && i < totalSlots && !s.img);
  ...
  function updateSlot(key: keyof SlotData, val: any) {
    setSlots(prev => prev.map((s, i) => i === activeSlot ? { ...s, [key]: val } : s));
  }
  ...
  function addOverlay(text: string, isEmoji = false) {
    ...
    setOverlays(prev => [...prev, {
      ...
      size:  isEmoji ? Math.round(H * 0.1) : textSize,
      color: textColor,
      angle: 0,
    }]);
  }
  ...
  function rotateSelected(deg: number) {
    ...
    setOverlays(prev => prev.map(o =>
      o.id === selected ? { ...o, angle: o.angle + deg } : o
    ));
  }
  ...
  function exportPrint() {
    ...
    ctx.scale(5, 5);
    // Re-draw at 1× into exp canvas by copying visible canvas
    ctx.drawImage(canvas, 0, 0, W, H);
    const dataUrl = exp.toDataURL('image/jpeg', 0.95);
    onComplete(dataUrl, slots);
  }

  const selectedOv = overlays.find(o => o.id === selected);

  const panelBtn = (id: typeof panel, label: string) => (
    ...
    <div style={{ width: '100%', maxWidth: 640 }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: 10 }}>
        ...
        <p style={{ fontSize: 12, color: '#888', margin: 0 }}>
          4×6 · up to 6 photos · drag overlays freely
        </p>
      </div>
      ...
      <div style={{ marginBottom: 10 }}>
        <p style={{ fontSize: 11, color: '#666', margin: '0 0 6px' }}>
          Layout — {filled}/{totalSlots} photos
        </p>
      ...
      {/* Canvas — the key: ref div for width, canvas fills it exactly */}
      <div ref={wrapRef} style={{
        width: '100%', borderRadius: 10, overflow: 'hidden',
        border: '1px solid #333', marginBottom: 10, background: '#111',
        touchAction: 'none',
      }}>
        <canvas
          ref={canvasRef}
      ...
      {selectedOv && (
        <div style={{
          ...
          <span style={{ fontSize: 11, color: '#888', flex: 1, alignSelf: 'center' }}>
            {selectedOv.text}
          </span>
          <button onClick={() => rotateSelected(-15)} style={{
            padding: '4px 7px', background: '#1a1a1a', border: '1px solid #333',
          ...
          <button onClick={deleteSelectedOverlay} style={{
            padding: '4px 7px', background: '#2a0a0a', border: '1px solid #A32D2D',
      ...
      <div style={{
        ...
        {panel === 'adjust' && (
          <div>
            <p style={{ fontSize: 11, color: '#666', margin: '0 0 8px' }}>
              Filter — slot {activeSlot + 1}
            </p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 5, marginBottom: 10 }}>
              {Object.keys(CSS_FILTERS).map(name => (
                <button key={name}
                  onClick={() => updateSlot('filter', CSS_FILTERS[name])}
                  style={{
                    padding: '5px 3px',
                    border: slots[activeSlot]?.filter === CSS_FILTERS[name]
                      ? '1px solid #4ADE80' : '1px solid #333',
                    borderRadius: 6,
                    background: slots[activeSlot]?.filter === CSS_FILTERS[name]
                      ? '#0d1f0d' : 'transparent',
                    color: slots[activeSlot]?.filter === CSS_FILTERS[name]
                      ? '#4ADE80' : '#888',
                    fontSize: 10, cursor: 'pointer',
                  }}>{name}</button>
              ))}
            ...
            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 10 }}>
              {Object.keys(CSS_FILTERS).map(name => (
                <button key={name}
                  onClick={() => {
                    setGlobalFilt(name);
                    setSlots(prev => prev.map(s => ({ ...s, filter: CSS_FILTERS[name] })));
                  }}
                  style={{
                    padding: '3px 7px', borderRadius: 14,
                    border: globalFilt === name ? '1px solid #4ADE80' : '1px solid #333',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
              {TEXT_PRESETS.map(t => (
                <button key={t}
                  ...
                  style={{
                    padding: '4px 8px', borderRadius: 14, border: '1px solid #333',
            ...
            <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
              <div style={{ flex: 1 }}>
                ...
                <input type="range" min={8} max={48} value={textSize}
            ...
            <button
              onClick={() => { if (textInput) { addOverlay(textInput); setTextInput(''); } }}
              disabled={!textInput}
              style={{
                padding: '10px', background: textInput ? '#4ADE80' : '#333',
                color: textInput ? '#000' : '#888', border: 'none',
        ...
        {panel === 'overlays' && (
          <div>
            <p style={{ fontSize: 11, color: '#666', margin: '0 0 8px' }}>
              Tap to add · drag on canvas · select to rotate
            </p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 5 }}>
              {CLIP_ART.map(e => (
                <button key={e} onClick={() => addOverlay(e, true)} style={{
                  padding: '7px 3px', borderRadius: 8, border: '1px solid #333',
                  background: '#1a1a1a', cursor: 'pointer', textAlign: 'center', fontSize: 18,
                }}>{e}</button>
              ))}
            </div>
          </div>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <button onClick={onBack} style={{
          flex: 1, padding: 12, border: '1px solid #333', borderRadius: 10,
