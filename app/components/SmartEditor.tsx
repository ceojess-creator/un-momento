...
export default function SmartEditor({
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      if (state.qrPlacement === 'corner_br' ||
        ...
        const x = state.qrPlacement === 'corner_br' ? PRINT_W - 66 : 16;
        const y = PRINT_H - 66;
  ...
  const tabStyle = (tab: string) => ({
    ...
    border: activeTab === tab ? '1px solid #444' : '1px solid transparent',
    borderRadius: 8,
  ...
  return (
    <div style={{ width: '100%', maxWidth: 640 }}>
      ...
      <div style={{
        ...
        {state.photoUrl ? (
          <canvas ref={canvasRef} width={PRINT_W} height={PRINT_H}
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 12 }}>
        {[
          ...
          { id: 'qr',      label: '⬛ QR'     },
          ...(showSticker ? [{ id: 'sticker', label: '✂️ Sticker' }] : []),
        ].map(tab => (
          <button key={tab.id}
      ...
      <div style={{
        ...
        {activeTab === 'photo' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <input style={inputStyle} placeholder="Graduate name"
        ...
        {activeTab === 'filter' && (
          <div>
            <div style={{
              ...
              {FILTERS.map(f => (
                <button key={f.name} onClick={() => set('filter', f.css)}
                  style={{
                    padding: '6px 4px',
                    background: state.filter === f.css ? '#0d1f0d' : 'transparent',
                    border: state.filter === f.css
                      ? '1px solid #4ADE80' : '1px solid #333',
                    borderRadius: 6,
                    color: state.filter === f.css ? '#4ADE80' : '#888',
            ...
            {[
              ...
              <div key={s.key} style={{ marginBottom: 10 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between',
                  ...
                  <span>{s.label}</span>
                  <span>{(state as any)[s.key]}%</span>
                </div>
                <input type="range" min={s.min} max={s.max}
                  value={(state as any)[s.key]}
                  onChange={e => set(s.key as any, Number(e.target.value))}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
              <div style={{ flex: 1 }}>
                ...
                <input type="range" min={14} max={48}
        ...
        {activeTab === 'qr' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <p style={{ fontSize: 12, color: '#888', margin: 0, lineHeight: 1.6 }}>
              ...
              Scannable from inside a frame — forever.
            </p>
            {QR_PLACEMENTS.map(p => (
              <div key={p.id} onClick={() => set('qrPlacement', p.id)}
                style={{
        ...
        {activeTab === 'sticker' && showSticker && (
          <StickerStudio
            onComplete={(slots, layout, shape) => {
              set('stickerSlots',  slots);
              set('stickerLayout', layout);
              set('stickerShape',  shape);
              setActiveTab('photo');
            }}
            onBack={() => setActiveTab('photo')}
          />
        )}
      </div>
