...
export default function StickerStudio({ onComplete, onBack }: StickerStudioProps) {
  ...
  function drawGuides(fc: any, layoutId: string, f: any) {
    ...
    [[4,4],[SHEET_W-20,4],[4,SHEET_H-20]].forEach(([rx,ry]) => {
      fc.add(new f.Rect({
        left:       rx, top: ry,
        width:      14, height: 14,
        fill:       '#000',
  ...
  function detectOverlaps(fc: any) {
    ...
    for (let i = 0; i < objs.length && !warned; i++) {
      for (let j = i + 1; j < objs.length && !warned; j++) {
        ...
        const ab = a.getBoundingRect(), bb = b.getBoundingRect();
        const ox = Math.max(0,
        ...
        if (overlap/(ab.width*ab.height) > 0.3 ||
            overlap/(bb.width*bb.height) > 0.3) {
          setOverlapWarn(
            `⚠️ "${a.customData?.label||a.type}" is covering ` +
            `"${b.customData?.label||b.type}" by more than 30%. ` +
  ...
  async function removeBackground() {
    ...
    if (!active?.customData?.file) {
      alert('Select a photo on the canvas first.');
      return;
    }
    setRemovingBg(true);
    try {
      ...
      if (data.url && fabricMod.current) {
        const img = await fabricMod.current.FabricImage.fromURL(
          data.url, { crossOrigin: 'anonymous' }
        );
        img.set({
          left:       active.left,
          top:        active.top,
  ...
  function addEmoji(emoji: string) {
    ...
    const text = new fabricMod.current.FabricText(emoji, {
      left:       SHEET_W / 2 + (Math.random() * 40 - 20),
      top:        SHEET_H / 2 + (Math.random() * 40 - 20),
      fontSize:   28,
      originX:    'center',
      originY:    'center',
  ...
  function bringForward() {
    ...
    if (!a) return;
    fabricRef.current.bringObjectForward(a);
    fabricRef.current.renderAll();
  }
  ...
  function sendBackward() {
    const a = fabricRef.current?.getActiveObject();
    if (!a) return;
    fabricRef.current.sendObjectBackwards(a);
    fabricRef.current.renderAll();
  }
  ...
  function handleComplete() {
    ...
    fabricRef.current.renderAll();
    const dataUrl = fabricRef.current.toDataURL({ format: 'png', multiplier: 4 });
    fabricRef.current.getObjects()
      .filter((o: any) => o.customData?.guide)
    ...
    <div style={{ width: '100%', maxWidth: 640 }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: 10 }}>
        ...
        <p style={{ fontSize: 12, color: '#888', margin: 0 }}>
          Drag freely · Pink dashed = die-cut · Black squares = registration marks
        </p>
      </div>
      ...
      {selectedObj && !selectedObj.customData?.guide && (
        <div style={{
          ...
          <span style={{ fontSize: 12, color: '#888', flex: 1, alignSelf: 'center' }}>
            {selectedObj.customData?.label || selectedObj.type}
          </span>
          <button onClick={bringForward} style={{
            padding: '4px 8px', background: '#1a1a1a',
            border: '1px solid #333', borderRadius: 6,
            color: '#fff', fontSize: 11, cursor: 'pointer',
          }}>↑ Forward</button>
          <button onClick={sendBackward} style={{
            padding: '4px 8px', background: '#1a1a1a',
            border: '1px solid #333', borderRadius: 6,
            color: '#fff', fontSize: 11, cursor: 'pointer',
          }}>↓ Back</button>
      ...
      <div style={{
        ...
        {activePanel === 'photo' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            }}>📷 Add photo to canvas</button>
            <input ref={fileInputRef} type="file" accept="image/*"
              style={{ display: 'none' }} onChange={handlePhotoUpload} />
            <button onClick={removeBackground} disabled={removingBg} style={{
              padding: '10px',
              background: removingBg ? '#333' : '#1a1a1a',
              color:      removingBg ? '#888' : '#fff',
              border: '1px solid #444', borderRadius: 8, fontSize: 13,
            ...
            <p style={{ fontSize: 11, color: '#555', margin: 0, lineHeight: 1.5 }}>
              1. Add photo → 2. Tap to select → 3. Remove background →
              4. Drag cutout anywhere. Repeat per slot.
            </p>
          </div>
        )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
              {TEXT_SUGGESTIONS.map(s => (
                <button key={s} onClick={() => setTextInput(s)} style={{
            ...
            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
              {FONTS.map(f => (
                <button key={f} onClick={() => setTextFont(f)} style={{
                  ...
                  border: textFont === f ? '1px solid #4ADE80' : '1px solid #333',
            ...
            <button onClick={addText} disabled={!textInput} style={{
              padding: '10px',
        ...
        {activePanel === 'overlays' && (
          <div>
            ...
            <div style={{
              ...
              {CLIP_ART.map(c => (
                <button key={c.emoji} onClick={() => addEmoji(c.emoji)} style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 6 }}>
                {LAYOUTS.map(l => (
                  <button key={l.id} onClick={() => handleLayoutChange(l.id)} style={{
                    padding: '8px',
                    border: layout === l.id ? '1px solid #4ADE80' : '1px solid #333',
                    borderRadius: 8,
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                {SHAPES.map(s => (
                  <button key={s.id} onClick={() => setShape(s.id)} style={{
                    flex: 1, padding: '8px',
                    border: shape === s.id ? '1px solid #4ADE80' : '1px solid #333',
                    borderRadius: 8,
            ...
            <p style={{ fontSize: 11, color: '#555', margin: 0, lineHeight: 1.6 }}>
              Pink dashed = Pixcut S1 die-cut path.
              Black squares = registration marks.
